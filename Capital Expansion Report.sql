...
-- ************************************** DC RESIDENT ********************************************************************
--enc DC resident
-- -- The below code will assign a client's residency status based on their city
-- -- (IF city name begins with wash then DC resident else not a DC resident)
-- -- This section of data is prone to error because of messy data entry, so it's worth giving a pass through to check for errors
SELECT 
...
SUM( CASE WHEN (#temptable1.language LIKE '%chinese%' OR #temptable1.language LIKE '%Manda%' OR #temptable1.language LIKE 'cantonese%')																					 THEN 1 ELSE 0 END) 'Chinese',
...
,sum( CASE WHEN #tempinsurance4.insurancename  LIKE ''			 THEN 1 ELSE 0 END)'Other Public' -- Not the best way to capture this info. Should this be a manual count
,sum( CASE WHEN 
				(
